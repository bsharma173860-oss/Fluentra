...
function MNotificationsPageV5() {
  ...
  const [readAll, setReadAll] = useStateMV5(false);
  const nav = (id) => window.__nav && window.__nav(id);
  ...
  const unread = readAll ? 0 : NOTIFS.filter(n => n.unread).length;
      ...
      <MobileBody padding={[0,16,30]} tabBarPad={false}>
        ...
        {tab === 'decks' && <>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 4px', marginBottom:8 }}>
            {V5_label('YOUR DECKS')}
            <button onClick={()=>window.__nav && window.__nav('lang')} style={{ fontSize:11, color:T.brand, fontWeight:700, background:'none', border:'none' }}>+ New</button>
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {decks.map(d => (
...
function MTutorPageV5() {
  ...
  const [showIntro, setShowIntro] = useStateMV5(true);
  ...
  async function send(preset) {
    var text = (preset != null ? String(preset) : (input || '')).trim(); if (!text) return;
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', background:T.bg }}>
      <MobileHeader title="Fluentra AI"/>
            ...
            <div key={i} style={{ display:'flex', flexDirection:'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
              ...
              {m.actions && (
                <div style={{ display:'flex', gap:6, marginTop:8, flexWrap:'wrap' }}>
                  {m.actions.map(a => <button key={a} onClick={()=>send(a)} style={{ padding:'7px 11px', borderRadius:99, background:T.brandLight, color:T.brand, fontSize:11.5, fontWeight:700, border:`1px solid ${T.brand}33` }}>{a}</button>)}
                </div>
              )}
