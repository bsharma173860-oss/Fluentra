    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:T.bg }}>
          ...
          <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'12px 16px' }}>
            <div style={{ width:'100%', background:T.card, border:`1px solid ${T.border}`, borderRadius:18, padding:'30px 22px', textAlign:'center', boxShadow:'0 6px 18px rgba(0,0,0,.04)' }}>
              ...
              <button aria-label="Play audio" onClick={()=>{ if (window.flSpeak && phrase) window.flSpeak(phrase.front, lang); }} style={{ width:60, height:60, borderRadius:'50%', background: mode==='speak'?T.speaking.c:T.brand, color:'#fff', display:'inline-flex', alignItems:'center', justifyContent:'center', boxShadow: mode==='speak' ? `0 8px 18px ${T.speaking.c}55` : `0 8px 18px ${T.brand}55`, cursor:'pointer', marginBottom:18, border:'none' }}>{mode==='speak' ? Icon.mic({ width:20, height:20 }) : Icon.play({ width:20, height:20 })}</button>
              {revealed ? (
