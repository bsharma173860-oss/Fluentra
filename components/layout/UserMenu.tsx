...
export function UserMenu({ name, email, plan }: UserMenuProps) {
  ...
  const trigger = (
    <TouchableOpacity style={t.row} onPress={() => setOpen(o => !o)} activeOpacity={0.8}>
      ...
      {open ? <ChevronUpIcon size={14} color={Colors.ink3} /> : <ChevronRightIcon size={14} color={Colors.ink3} />}
    </TouchableOpacity>
  );
...
const t = StyleSheet.create({
  ...
  name:       { fontFamily: 'Inter_500Medium',  fontSize: 13, color: Colors.ink  },
  plan:       { fontFamily: 'Inter_400Regular', fontSize: 11, color: Colors.ink3 },
});
