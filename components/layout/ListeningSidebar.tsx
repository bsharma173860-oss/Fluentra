...
import { ChevronLeftIcon, HeadphoneIcon } from '@/components/icons';
...
export function ListeningSidebar() {
  ...
  async function loadData() {
    ...
    const [sessRes, profileRes] = await Promise.all([
      supabase.from('listening_sessions')
        .select('id, section, score, topic, duration_seconds, created_at')
        .eq('user_id', user.id)
  ...
  const SECTIONS = [
    ...
    { key: 'full', label: 'Full Test — All 4'      },
  ];
  ...
  return (
    <View style={s.sidebar}>
      ...
      <View style={s.titleRow}>
        <Text style={s.title}>Listening</Text>
      ...
      <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}>
          ...
          {SECTIONS.map(sec => (
            <TouchableOpacity
              ...
              onPress={() => router.push({
                ...
                params: { section: sec.key, languageCode: code, code },
              })}
              activeOpacity={0.8}
            >
              <HeadphoneIcon size={13} color={GREEN} />
              <Text style={s.sessionBtnText}>{sec.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={s.group}>
          {sessions.length === 0 ? (
            <Text style={s.emptyText}>No listening sessions yet</Text>
          ) : (
            sessions.map(sess => (
              <View key={sess.id} style={s.histRow}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={s.histSection} numberOfLines={1}>
                    {sess.section === 'full' ? 'Full Test' : `Section ${sess.section}`}
                    {sess.topic ? ` · ${sess.topic.slice(0, 16)}…` : ''}
                  </Text>
                  <Text style={s.histDate}>{formatDate(sess.created_at)}</Text>
                </View>
                {sess.score != null && (
                  <Text style={s.histScore}>{sess.score.toFixed(1)}</Text>
                )}
              </View>
            ))
          )}
...
const s = StyleSheet.create({
  ...
  sessionBtn: {
    ...
    backgroundColor: GREEN_BG, borderWidth: 1, borderColor: GREEN_BDR,
    borderRadius: 10, paddingVertical: 9, paddingHorizontal: 12,
  },
  sessionBtnText: { fontFamily: 'Inter_500Medium', fontSize: 12, color: GREEN },
  histRow:    { flexDirection: 'row', alignItems: 'center', gap: 9, paddingHorizontal: 4, paddingVertical: 5 },
  histDot:    { width: 6, height: 6, borderRadius: 3, backgroundColor: GREEN, flexShrink: 0 },
  ...
  histScore:  { fontFamily: 'Inter_700Bold', fontSize: 12, color: GREEN },
  ...
  bestCard: {
    ...
    backgroundColor: GREEN_BG, padding: 12, alignItems: 'center', gap: 2,
  },
  bestLabel: { fontFamily: 'Inter_700Bold', fontSize: 11, color: GREEN, textTransform: 'uppercase' as const, letterSpacing: 0.5 },
  bestNum:   { fontFamily: 'Inter_700Bold', fontSize: 24, color: GREEN },
