    ...
    <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
        ...
        <Field label="Password" type={showPw ? 'text' : 'password'} placeholder="••••••••" value={pw} onChange={setPw}
          ...
          hint={<a href="#" style={{ fontSize:11.5, color:T.brand, fontWeight:600, textDecoration:'none' }} onClick={e => { e.preventDefault(); window.__nav && window.__nav('forgot_pw'); }}>Forgot?</a>}
        />
      </div>

      {error && <div style={{ fontSize:12.5, color:'#C0392B', background:'#FFF0EE', borderRadius:10, padding:'10px 14px', lineHeight:1.5 }}>{error}</div>}
      ...
      <div style={{ textAlign:'center', fontSize:13, color:T.ink3 }}>
        No account? <span style={{ color:T.brand, fontWeight:700, cursor:'pointer' }} onClick={() => window.__nav && window.__nav('auth_signup')}>Sign up free</span>
      </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:28, paddingBottom:40 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <Field label="Password" type="password" placeholder="••••••••" value={pw} onChange={setPw}
              right={<a href="#" style={{ fontSize:11.5, color:T.brand, fontWeight:600, textDecoration:'none' }} onClick={e => { e.preventDefault(); window.__nav && window.__nav('forgot_pw'); }}>Forgot?</a>}
            />
          </div>
          {error && <div style={{ fontSize:12, color:'#C0392B', background:'#FFF0EE', borderRadius:10, padding:'9px 12px' }}>{error}</div>}
          <Btn label={loading ? 'Signing in…' : 'Sign in'} fullWidth accent={T.brand} size="lg" onClick={handleSignIn}/>
          <div style={{ textAlign:'center', fontSize:13, color:T.ink3 }}>
            No account? <span style={{ color:T.brand, fontWeight:700, cursor:'pointer' }} onClick={() => window.__nav && window.__nav('auth_signup')}>Sign up free</span>
          </div>
...
function SignupCard() {
  ...
  if (success) return (
    <div style={{ display:'flex', flexDirection:'column', gap:20, alignItems:'center', textAlign:'center', paddingTop:40 }}>
      <AuthLogo/>
      <div style={{ width:64, height:64, borderRadius:32, background:'#E8F8EE', display:'flex', alignItems:'center', justifyContent:'center', fontSize:32 }}>✅</div>
      <div style={{ fontSize:20, fontWeight:700, color:T.ink }}>Check your email</div>
      <div style={{ fontSize:13.5, color:T.ink3, lineHeight:1.6, maxWidth:320 }}>We sent a confirmation link to <strong>{email}</strong>. Click it to activate your account, then come back and sign in.</div>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:22 }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        <SocialBtn icon={<GoogleIcon/>} label="Sign up with Google" onClick={handleGoogle}/>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
        <div style={{ display:'flex', gap:5 }}>
          {[1,2,3,4].map(i => (
            <div key={i} style={{ flex:1, height:4, borderRadius:99, background: pw.length >= i*2 ? (i < 3 ? T.writing : i < 4 ? T.listening : T.brand) : T.bg3 }}/>
          ))}
        </div>
      ...
      {error && <div style={{ fontSize:12.5, color:'#C0392B', background:'#FFF0EE', borderRadius:10, padding:'10px 14px' }}>{error}</div>}

      <Btn label={loading ? 'Creating account…' : 'Create account'} fullWidth accent={T.brand} size="lg" onClick={handleSignUp}/>

      <div style={{ textAlign:'center', fontSize:13, color:T.ink3 }}>
        Already have an account? <span style={{ color:T.brand, fontWeight:700, cursor:'pointer' }} onClick={() => window.__nav && window.__nav('auth_login')}>Sign in</span>
      </div>
    </div>
  );
}
  ...
  if (success) return (
    <MobileBody>
      <div style={{ display:'flex', flexDirection:'column', gap:18, alignItems:'center', textAlign:'center', paddingTop:32, paddingBottom:40 }}>
        <AuthLogo size="sm"/>
        <div style={{ width:56, height:56, borderRadius:28, background:'#E8F8EE', display:'flex', alignItems:'center', justifyContent:'center', fontSize:28 }}>✅</div>
        <div style={{ fontSize:18, fontWeight:700, color:T.ink }}>Check your email</div>
        <div style={{ fontSize:13, color:T.ink3, lineHeight:1.6 }}>We sent a confirmation link to <strong>{email}</strong>. Click it to activate your account.</div>
        <Btn label="Go to sign in" fullWidth accent={T.brand} size="lg" onClick={() => window.__nav && window.__nav('auth_login')}/>
      </div>
    </MobileBody>
  );
  ...
  return (
    <MobileBody>
      <div style={{ display:'flex', flexDirection:'column', gap:22, paddingBottom:40 }}>
        ...
        <Btn label={loading ? 'Creating account…' : 'Create account'} fullWidth accent={T.brand} size="lg" onClick={handleSignUp}/>
        <div style={{ textAlign:'center', fontSize:13, color:T.ink3 }}>
          Already have one? <span style={{ color:T.brand, fontWeight:700, cursor:'pointer' }} onClick={() => window.__nav && window.__nav('auth_login')}>Sign in</span>
        </div>
